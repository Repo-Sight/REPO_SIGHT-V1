...
std::vector<Violation> checkTypeScriptRules(const std::string& path, const std::vector<Token>& tokens,
    ...
    for (std::size_t i = 0; i < n; ++i) {
        ...
        if (tok.type == TokenType::OPERATOR && tok.value == "." &&
            ...
            out.push_back(makeSecurityViolation(path, tokens[i + 1].line, "ts-sec-inner-html",
        ...
        // ts-sec-weak-hash: crypto.createHash('md5'/'sha1') is broken for
        ...
        if (tok.type == TokenType::IDENTIFIER && tok.value == "createHash" &&
            ...
            if (algo.find("md5") != std::string::npos || algo.find("sha1") != std::string::npos) {
                out.push_back(makeSecurityViolation(path, tok.line, "ts-sec-weak-hash",
        ...
        // ts-sec-disabled-tls: 'rejectUnauthorized: false' disables TLS
        ...
        if (tok.type == TokenType::IDENTIFIER && tok.value == "rejectUnauthorized" &&
            ...
            out.push_back(makeSecurityViolation(path, tok.line, "ts-sec-disabled-tls",
                "'rejectUnauthorized: false' disables TLS certificate validation -- review "
                "whether this can reach production", "warning"));
        }

        // ts-sec-sql-string-concat: query(...) with a '+' inside the call is
        ...
        if (tok.type == TokenType::IDENTIFIER && tok.value == "query" &&
            ...
            if (sawConcat) {
                out.push_back(makeSecurityViolation(path, tok.line, "ts-sec-sql-string-concat",
                    "'query(...)' builds its statement with string concatenation -- review for "
                    "SQL injection, prefer parameterized/tagged queries", "warning"));
            }
        }

        // ts-sec-hardcoded-secret: NAME = "literal" where NAME looks like a
        ...
        if (tok.type == TokenType::IDENTIFIER && isSuspiciousSecretName(tok.value) &&
            ...
            out.push_back(makeSecurityViolation(path, tok.line, "ts-sec-hardcoded-secret",
